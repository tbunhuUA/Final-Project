...
parser.add_argument('-d', '--display', help='Displays all the items of the todo list')
parser.add_argument('-i', '--id', type=int, help='An input item used to call a specific value by the id')
parser.add_argument('-a', '--add', help='Adds a new item to the todo list')
...
if args.add:
        ...
        with open(file, 'a') as todo:
            todo.write('\n' + str(idCount) + '\n')
            todo.write(newStatus + '\n')
            todo.write(newCategory + '\n')
            todo.write(newDescription)

if args.status:
    if args.status.lower() == 'p' or args.status.lower() == 'in-progress':
